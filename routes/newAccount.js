        ...
        //succcess case
            ...
            database.connect(db=>{
              //update users and stripe accounts
                db.db('users').collection('stripe_users').update({'username':req.session.key}, {$set:{'username':req.session.key, 'stripe_connected_account_id':stripe_id, 'payouts':[]}}, {upsert:true}, (err20, res4)=>{
                    if (err20){
